...
class Evaluator:
    ...
    def __init__(
        self, model_dir, generator_model_name, discriminator_model_name, device="cpu"
    ):
        self.device = device
        ...
        self.generator = Generator(
            ...
            tokenizer_name=generator_model_name,
            device=device,
        )
        self.discriminator = Discriminator(
            model_dir=model_dir,
            model_name=discriminator_model_name,
            tokenizer_name=generator_model_name,
            device=device,
        )
    ...
    def evaluate(self, data_loader):
        ...
        with torch.no_grad():
            for batch in data_loader:
                ...
                fake_answers, _ = self.generator.generate(
                ...
                all_answers = torch.cat(
                    (batch["answer_input_ids"], fake_answers), dim=0
                )
                all_attention_masks = torch.cat(
                    (batch["answer_attention_mask"], batch["answer_attention_mask"]),
                    dim=0,
                )
                all_token_type_ids = torch.cat(
                    (batch["answer_token_type_ids"], batch["answer_token_type_ids"]),
                    dim=0,
                )

                # Labels for real and fake answers
                real_labels = torch.ones(
                    batch["answer_input_ids"].shape[0], dtype=torch.long
                ).to(self.device)
                ...
                discriminator_logits = self.discriminator(
                    all_answers, all_attention_masks, all_token_type_ids
                )

                # Compute discriminator loss and accuracy
                discriminator_loss = (
                    torch.nn.functional.binary_cross_entropy_with_logits(
                        discriminator_logits, all_labels.float()
                    )
                )
                discriminator_predictions = torch.round(
                    torch.sigmoid(discriminator_logits)
                )
                discriminator_correct = (
                    (discriminator_predictions == all_labels).sum().item()
                )
